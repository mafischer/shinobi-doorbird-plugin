...
Shinobi.cx = function(x){return socket.emit('ocv',x)};
// socket connect callback
socket.on('connect', function(d){
    Shinobi.cx({f:'init',plug:'doorbird'});
});
...
const rstpQuery = "select mid, ke, host, details from `ccio`.`Monitors` where path = '/mpeg/media.amp';";
    ...
    async.each(doorbirds, function (doorbird, callback){
        ...
        async.each(config.events, function(event, callback){
            request('http://' + doorbird.host + '/bha-api/notification.cgi?url=http://' + hostname + ':3000/doorbird/event/' + event + '/monitor/' +
                doorbird.id + '/group/' + doorbird.group + '&subscribe=1&relaxation=10&event=' + event,
