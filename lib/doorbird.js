...
async.parallel({
    ...
    async.each(doorbirds, function (doorbird, callback){
        ...
        async.each(config.events, function(event, callback){
            request('http://' + doorbird.host + '/bha-api/notification.cgi?url=http://' + config.host + ':3000/doorbird/event/' + event + '/monitor/' +
                doorbird.id + '&subscribe=1&relaxation=10&event=' + event,
...
exports.relay = function relay(message, callback){
    console.log("relay message: " + message.event);

    // // socket connect callback
    // socket.on('connect', function(d){
    //     cx({f:'init',plug:'doorbird'});
    // });
    //
...
function cx(x){return socket.emit('ocv',x)}
